...
def load_arguments(self, _):
    ...
    configuration_file_type = CLIArgumentType(options_list='--gaming-configuration-file', help='Configuration File', id_part='file', required=False)

    # Service Principal
    gaming_auth_type = CLIArgumentType(options_list='--gaming-auth-sp', help='Service Principal ID.', id_part='sp', required=False)
    gaming_auth_secret_type = CLIArgumentType(options_list='--gaming-auth-secret', help='Service Principal Secret.', id_part='secret', required=False)
    gaming_auth_tenent_type = CLIArgumentType(options_list='--gaming-auth-tenent', help='Active Directory Tenent', id_part='tenent', required=False)
    
    # User
    gaming_auth_scope_type = CLIArgumentType(options_list='--gaming-auth-scope', help='Authentication Scope', id_part='scope', required=True)

    # Platform - alsu uses gaming auth type
    gaming_auth_refresh_type = CLIArgumentType(options_list='--gaming-auth-refresh', help='User Refersh token', id_part='refresh', required=True)
    gaming_auth_portal_type = CLIArgumentType(options_list='--gaming-auth-platform', help='Experience Lab Portal Url', id_part='platform', required=True)


    configuration_section_platform = CLIArgumentType(options_list='--gaming-configuration-platform', help='Configuration Platform', id_part='platform', required=True)
    configuration_section_tool = CLIArgumentType(options_list='--gaming-configuration-tool', help='Configuration Tool', id_part='tool', required=True)
    ...
    with self.argument_context('{} auth sp'.format(CommandConstants.ROOT), validator=acquire_token_validator) as c:
        ...
        c.argument('service_principal_secret', gaming_auth_secret_type, options_list=['--service-principal-credential', '-c'])
    ...
    # Platform Config
    ##############################################
    with self.argument_context('{} platform add'.format(CommandConstants.ROOT), validator=configuration_file_validator) as c:
        c.argument('configuration_file', configuration_file_type, options_list=['--configuration', '-f'])
    ...
    # Tool Config
    ##############################################
    with self.argument_context('{} utility add'.format(CommandConstants.ROOT), validator=configuration_file_validator) as c:
        c.argument('configuration_file', configuration_file_type, options_list=['--configuration', '-f'])
